    ...
    public static function set($name, $value = '', $option = null)
    {
        ...
        if ($config['setcookie']) {
            if (isset($_SERVER['SWOOLE_CLI'])){
                /**
                 * @var \swoole_http_response $swoole_response
                 */
                $swoole_response = $_SERVER['SWOOLE_RESPONSE'];
                $swoole_response->cookie($name, $value, $expire, $config['path'], $config['domain'],
    ...
    public static function delete($name, $prefix = null)
    {
        ...
        if ($config['setcookie']) {
            if (isset($_SERVER['SWOOLE_CLI'])){
                 ...
                 */
                $swoole_response = $_SERVER['SWOOLE_RESPONSE'];
                $swoole_response->cookie($name, '', $_SERVER['REQUEST_TIME'] - 3600, $config['path'],
                ...
                if (0 === strpos($key, $prefix)) {
                    if ($config['setcookie']) {
                        if (isset($_SERVER['SWOOLE_CLI'])){
                             ...
                             */
                            $swoole_response = $_SERVER['SWOOLE_RESPONSE'];
                            $swoole_response->cookie($key, '', $_SERVER['REQUEST_TIME'] - 3600, $config['path'],
                                $config['domain'], $config['secure'], $config['httponly']);
                        }else{
                            setcookie(
